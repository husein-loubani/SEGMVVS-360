...
def extract_skyline(binary_mask, spike_thresh=20):
    """Extract cleaned skyline polyline from binary mask."""
    height, width = binary_mask.shape
    y_vals = np.full(width, height, dtype=np.int32)

    for x in range(width):
        col = binary_mask[:, x]
        y_indices = np.where(col > 0)[0]
        if y_indices.size > 0:
            y_vals[x] = y_indices[0]

    filtered_points = []
    for x in range(1, width - 1):
        y_prev, y_curr, y_next = y_vals[x - 1], y_vals[x], y_vals[x + 1]
        local_avg = (y_prev + y_next) / 2
        if abs(y_curr - local_avg) < spike_thresh:
            filtered_points.append((x, y_curr))

    skyline = np.zeros_like(binary_mask)
    if len(filtered_points) > 1:
        points_array = np.array(filtered_points, dtype=np.int32)
        cv2.polylines(skyline, [points_array], isClosed=False, color=255, thickness=1)
...
def segment_real_image(req):
    try:
        cv_img = bridge.imgmsg_to_cv2(req.real_rgb_image, desired_encoding="rgb8")
        ...
        skyline = extract_skyline(resized_mask)
        ...
        binary_ros_img = bridge.cv2_to_imgmsg(resized_mask, encoding="mono8")
        skyline_ros_img = bridge.cv2_to_imgmsg(skyline, encoding="mono8")
        semantic_ros_img = bridge.cv2_to_imgmsg(semantic_resized, encoding="rgb8")
        dynamic_ros_img = bridge.cv2_to_imgmsg(dynamic_mask, encoding="mono8")

        return building_segmentationResponse(
            real_binary_mask_image=binary_ros_img,
            real_skyline_image=skyline_ros_img,
...
if __name__ == "__main__":
    ...
    rospy.Service("building_segmentation", building_segmentation, segment_real_image)
    rospy.spin()
